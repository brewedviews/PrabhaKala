      ...
      <section className="container pt-8 pb-16 grid grid-cols-1 lg:grid-cols-2 gap-8 lg:gap-16">
        ...
        <div className="grid grid-cols-[80px_1fr] gap-4">
          ...
          <div className="relative aspect-[3/4] overflow-hidden bg-brand-cream cursor-zoom-in" onClick={() => setZoom(!zoom)}>
            <img src={product.images[activeImg]?.url} alt={product.title} className={`w-full h-full object-cover transition-transform duration-500 ${zoom ? 'scale-150' : ''}`} />
            {disc > 0 && (
        ...
        <div>
          ...
          <div className="flex items-baseline gap-4 mt-6">
            <span className="font-serif text-3xl text-brand-maroon">{formatINR(price)}</span>
            {disc > 0 && (
              <>
                <span className="text-lg text-brand-brown/40 line-through">{formatINR(compareAt)}</span>
                <span className="text-sm text-brand-gold-dark font-medium">You save {formatINR(compareAt - price)}</span>
              </>
            )}
          </div>
          ...
          <div className="grid grid-cols-2 gap-y-3 text-sm mb-8">
            ...
            <div className={stock > 0 ? 'text-emerald-700' : 'text-red-700'}>{stock > 0 ? `In Stock (${stock})` : 'Out of Stock'}</div>
          </div>
          ...
          <div className="flex items-center gap-4 mb-4">
            <div className="flex items-center border border-brand-brown/30">
              ...
              <button onClick={() => setQty(Math.min(stock || 1, qty + 1))} className="px-3 py-3 hover:bg-brand-cream"><Plus size={14} /></button>
            </div>
            <button
              onClick={handleAdd}
            ...
            <button onClick={() => toggleWishlist(product.handle)} className="h-[52px] w-[52px] border border-brand-brown/30 flex items-center justify-center hover:bg-brand-cream" aria-label="Wishlist">
      ...
      {similar.length > 0 && (
        <section className="container pb-24">
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 gap-6">
            {similar.map((p) => <ProductCard key={p.handle} product={p} />)}
          </div>
        </section>
      )}
    </>

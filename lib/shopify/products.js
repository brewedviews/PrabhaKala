...
import { PRODUCTS_QUERY, PRODUCT_BY_HANDLE_QUERY, PRODUCT_RECOMMENDATIONS_QUERY } from './queries';
...
export function normalizeProduct(node) {
  ...
  return {
    ...
    compareAtPriceRange: {
      min: normalizeMoney(node.compareAtPriceRange?.minVariantPrice),
      max: normalizeMoney(node.compareAtPriceRange?.maxVariantPrice),
    },
    variants: (node.variants?.edges || []).map((e) => ({
      ...
      compareAtPrice: normalizeMoney(e.node.compareAtPrice),
